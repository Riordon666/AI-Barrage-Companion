...
当前项目基本处于空白启动状态，仓库里还没有可复用的业务模块。问题原因是项目目前只有示例脚本，尚未建立桌面应用、截屏分析、AI 调用、弹幕调度和配置管理等基础结构。
...
- `codex-text/` 只存放策划、审查、规划、记录等非项目运行必需文档。
...
- 创建 `app/`、`core/`、`ui/`、`config/`、`tests/` 目录。
- 新增 `requirements.txt`，包含 PySide6、mss、requests 或 httpx、pytest。
- 用 `app/main.py` 作为应用入口。
- 保留无 API Key 可运行的模拟模式。
- 新增 `interfaces.py`，先定义 `BarrageGenerator`、`OverlayRenderer`、`PrivacyGuard` 等协议边界。
